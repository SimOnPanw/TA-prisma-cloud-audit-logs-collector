...
def validate_input(helper, definition):
    ...
    payload = json.dumps({"username": opt_access_key, "password": opt_secret_key})
    ...
    if r_status != 200:
        raise ValueError(
            "Authentication failed! Please verify base url, access key and the secret key. Return code is %s", str(r_status)
        )
...
def collect_events(helper, ew):
    ...
    if not token:
        ...
        status = extend_token(helper, opt_base_url, token)
        if status != 200:
    ...
    ## GET AUDIT LOGS
    audit_logs = get_audit_logs(helper, opt_base_url, opt_time_amount, opt_time_unit, token)
...
def extend_token(helper, opt_base_url, token):
    ## EXTEND TOKEN IF IT EXISTS
    url = "https://%s/auth_token/extend" % (opt_base_url)
    # helper.log_info("AUDIT_LOGS FROM EVENT ==URL: " + url)
...
def get_audit_logs(helper, opt_base_url, opt_time_amount, opt_time_unit, token):
