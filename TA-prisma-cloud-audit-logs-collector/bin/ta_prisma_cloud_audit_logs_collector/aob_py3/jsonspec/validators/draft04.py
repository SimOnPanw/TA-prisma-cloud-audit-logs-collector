...
def compile(schema, pointer, context, scope=None):
    ...
    if "additionalProperties" in schm:
        ...
        if isinstance(attrs["additional_properties"], dict):
            compiled = compile(attrs["additional_properties"], subpointer, context, scope)
                    ...
                    with self.catch_fail():
                        try:
                            ...
                            if additionals is True:
                                ...
                                self.fail("Forbidden value", obj, pointer=pointer_join(self.uri, index))  # noqa
                                continue
                            validator = additionals
                        validated[index] = validator(element, pointer_join(pointer, index))  # noqa
