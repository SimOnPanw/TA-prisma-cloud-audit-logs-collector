...
class _PRecordMeta(type):
    def __new__(mcs, name, bases, dct):
        ...
        dct["_precord_initial_values"] = dict(
...
class PRecord(PMap, CheckedType, metaclass=_PRecordMeta):
    ...
    def __new__(cls, **kwargs):
        ...
        e = _PRecordEvolver(
            cls, pmap(pre_size=len(cls._precord_fields)), _factory_fields=factory_fields, _ignore_extra=ignore_extra
        )
    ...
    def persistent(self):
        ...
        if cls._precord_mandatory_fields:
            self._missing_fields += tuple(
                "{0}.{1}".format(cls.__name__, f) for f in (cls._precord_mandatory_fields - set(result.keys()))
            )
