...
def field(
    ...
    field = _PField(
        type=types, invariant=invariant_function, initial=initial, mandatory=mandatory, factory=factory, serializer=serializer
    )
...
def _sequence_field(checked_class, item_type, optional, initial):
    ...
    return field(
        type=optional_type(TheType) if optional else TheType, factory=factory, mandatory=True, initial=factory(initial)
    )
...
def _make_pmap_field_type(key_type, value_type):
    ...
    TheMap.__name__ = "{0}To{1}PMap".format(
        _types_to_names(TheMap._checked_key_types), _types_to_names(TheMap._checked_value_types)
    )
