    ...
    def decorator(fn):
        def wrapper(path, name, op, *args, **kwargs):
            ...
            if path not in spec.paths:
                spec.add_path(path)
            param = {"name": "id", "in": "path", "required": True, "type": "string"}
...
def api_body_param(is_model_class_used, ref, is_list=False):
    ...
    def decorator(fn):
        def wrapper(path, name, op, *args, **kwargs):
            if not spec.paths:
                return fn(path, name, op, *args, **kwargs)
            param = {"name": "body", "in": "body", "required": True}
            if is_list:
                param["schema"] = {
                    "type": "array",
    ...
    def decorator(fn):
        def wrapper(path, name, op, *args, **kwargs):
            if not spec.paths:
                return fn(path, name, op, *args, **kwargs)
            for k in params:
                param = {"name": k, "in": "query", "required": False, "type": "string"}
                # add parameter to operation
                op["parameters"].append(param)
...
def _generate_documentation(context, method_list):
    ...
    for method in method_list:
        try:
            _rest_client.request(uri, owner=context.get("session"), method=method)
...
class _SwaggerSpecGenerator(object):
    ...
    def update_spec(self):
        ...
        try:
            os.rename(
                tempfile.gettempdir() + op.sep + "temp.yaml",
