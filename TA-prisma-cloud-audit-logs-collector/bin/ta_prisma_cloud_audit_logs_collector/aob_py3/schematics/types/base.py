...
class DateTimeType(BaseType):
    ...
    def __init__(
        self, formats=None, serialized_format=None, parser=None, tzd="allow", convert_tz=False, drop_tzinfo=False, **kwargs
    ):
    ...
    def _mock(self, context=None):
        ...
        if self.tzd == "reject" or self.drop_tzinfo or self.tzd == "allow" and random.randrange(2):
            ...
            return dt.replace(tzinfo=self.offset_timezone(hours=random.randrange(-12, 15), minutes=random.choice([0, 30, 45])))
    ...
    def from_string(self, value):
        ...
        if "tzd_utc" in parts:
            ...
            tz = None
        try:
            return datetime.datetime(p("year"), p("month"), p("day"), p("hour"), p("minute"), p("second"), microsecond, tz)
        except (ValueError, TypeError):
            return None
...
class MultilingualStringType(BaseType):
    ...
    def __init__(self, regex=None, max_length=None, min_length=None, default_locale=None, locale_regex=LOCALE_REGEX, **kwargs):
        self.regex = re.compile(regex) if regex else None
        self.max_length = max_length
        self.min_length = min_length
