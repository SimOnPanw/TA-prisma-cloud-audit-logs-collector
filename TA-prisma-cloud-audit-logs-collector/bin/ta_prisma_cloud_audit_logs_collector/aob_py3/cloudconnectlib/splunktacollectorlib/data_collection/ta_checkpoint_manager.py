...
class TACheckPointMgr(object):
    ...
    def __init__(self, meta_config, task_config):
        ...
        self._store = self._create_state_store(meta_config, task_config.get(c.checkpoint_storage_type), task_config[c.appname])
    ...
    def _create_state_store(self, meta_config, storage_type, app_name):
        ...
        if storage_type == c.checkpoint_kv_storage:
            ...
            stulog.logger.debug("Creating KV state store, collection name=%s", collection_name)
            return ss.get_state_store(meta_config, appname=app_name, collection_name=collection_name, use_kv_store=True)

        use_cache_file = self._use_cache_file()
        max_cache_seconds = self._get_max_cache_seconds() if use_cache_file else None

        stulog.logger.debug(
            "Creating file state store, use_cache_file=%s, max_cache_seconds=%s", use_cache_file, max_cache_seconds
        )

        return ss.get_state_store(meta_config, app_name, use_cached_store=use_cache_file)
    ...
    def _get_collection_name(self):
        ...
        if not collection:
            ...
            stulog.logger.info('Collection name="%s" is empty, set it to "%s"', collection, input_name)
    ...
    def _use_cache_file(self):
        ...
        if use_cache_file:
            stulog.logger.info("Stanza=%s using cached file store to create checkpoint", self._task_config[c.stanza_name])
    ...
    def _get_max_cache_seconds(self):
        ...
        try:
            ...
            stulog.logger.warning(
                "The max_cache_seconds '%s' is not a valid integer," " so set this variable to default value %s",
                seconds,
                default,
            )
            seconds = default
            ...
            if not (1 <= seconds <= maximum):
                ...
                stulog.logger.warning(
                    "The max_cache_seconds (%s) is expected in range[1,%s]," " set it to %s", seconds, maximum, adjusted
                )
                seconds = adjusted
        return seconds
    ...
    def get_ckpt(self, namespaces=None, show_namespaces=False):
        ...
        stulog.logger.info("Get checkpoint key='%s' value='%s'", key, json.dumps(raw_checkpoint))
    ...
    def update_ckpt(self, ckpt, namespaces=None):
        ...
        stulog.logger.info("Update checkpoint key='%s' value='%s'", key, json.dumps(value))
