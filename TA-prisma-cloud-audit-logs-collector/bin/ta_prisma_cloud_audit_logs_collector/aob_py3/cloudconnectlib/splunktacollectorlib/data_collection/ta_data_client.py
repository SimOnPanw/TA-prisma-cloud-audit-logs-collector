...
def build_event(host=None, source=None, sourcetype=None, time=None, index=None, raw_data="", is_unbroken=False, is_done=False):
...
class TaDataClient(object):
    def __init__(self, meta_config, task_config, checkpoint_mgr=None, event_writer=None):
...
def create_data_collector(dataloader, tconfig, meta_configs, task_config, data_client_cls, checkpoint_cls=None):
    ...
    return tdc.TADataCollector(tconfig, meta_configs, task_config, checkpoint_manager_cls, data_client_cls, dataloader)
...
def client_adatper(job_func):
    class TaDataClientAdapter(TaDataClient):
        def __init__(self, all_conf_contents, meta_config, task_config, chp_mgr):
            super(TaDataClientAdapter, self).__init__(meta_config, task_config, chp_mgr)
