...
from ..common.lib_util import get_main_file, get_app_root_dir, get_mod_input_script_name
...
def _find_ucc_global_config_json(app_root, ucc_config_filename):
    ...
    for candidate in candidates:
        ...
        if op.isfile(file_path):
            return file_path
    raise RuntimeError("Unable to load %s from [%s]" % (ucc_config_filename, ",".join(candidates)))
