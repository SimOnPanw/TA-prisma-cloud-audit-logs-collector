...
class Config(object):
    ...
    META_FIELDS = ("_product", "_rest_namespace", "_rest_prefix", "_protocol_version", "_version", "_encryption_formatter")
        ...
        for ep_id, ep in self._endpoints.items():
            ...
            for retry in range(retries):
                resp, cont = splunkd_request(
                    splunkd_uri=self.make_uri(ep_id), session_key=self.session_key, data=data, retry=3
                )
    ...
    def update_items(self, endpoint_id, item_names, field_names, data, raise_if_failed=False):
        ...
        for item_name in item_names:
            ...
            resp, cont = splunkd_request(
                splunkd_uri=item_uri, session_key=self.session_key, data=item_data, method="POST", retry=3
            )
            if resp is None or resp.status not in (200, 201):
                msg = 'Fail to update item "{item}" in endpoint "{ep_id}"' " - {err}".format(
                    ep_id=endpoint_id, item=item_name, err=code_to_msg(resp, cont) if resp else cont
                )
    ...
    def make_uri(self, endpoint_id, item_name=None):
        ...
        ep_uri = (
            ...
            "".format(splunkd_uri=self.splunkd_uri, user=self.user, app=self.app, endpoint_full=ep_full)
        )
    ...
    def _parse_content(self, endpoint_id, content):
        ...
        try:
            ...
            msg = "Fail to parse content from endpoint_id=%s" " - %s" % (endpoint_id, exc)
            log(msg, level=logging.ERROR, need_tb=True)
            raise ConfigException(msg)
    ...
    def _parse_schema(self, ucc_config_schema):
        ...
        for field in Config.META_FIELDS:
            assert field in ucc_config_schema and isinstance(ucc_config_schema[field], six.string_types), (
    ...
    def _check_protocol_version(self):
        ...
        if not self._protocol_version.startswith("1."):
            raise ConfigException('Unsupported protocol version "%s" ' "in given schema" % self._protocol_version)
    ...
    def load_value(self, endpoint_id, item_name, fname, fval):
        ...
        try:
            ...
            msg = (
                ...
                "".format(type_name=field_type, endpoint=endpoint_id, item=item_name, field=fname)
            )
