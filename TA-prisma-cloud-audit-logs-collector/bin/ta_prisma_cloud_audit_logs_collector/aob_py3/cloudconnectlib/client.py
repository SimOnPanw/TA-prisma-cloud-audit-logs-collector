...
class CloudConnectClient(object):
    ...
    def _load_config(self):
        ...
        try:
            ...
            raise ConfigException("Unable to load configuration file %s: %s" % (self._config_file, traceback.format_exc()))

        version = conf.get("meta", {"apiVersion", None}).get("apiVersion", None)
        if not version:
            raise ConfigException("Config meta or api version not present in {}".format(self._config_file))

        config_loader, schema_file = get_loader_by_version(version)
        schema_path = os.path.join(os.path.dirname(__file__), "configuration", schema_file)
    ...
    def start(self):
        ...
        try:
            ...
            self._engine.start(context=copy.deepcopy(self._context), config=self._config, checkpoint_mgr=self._checkpoint_mgr)
