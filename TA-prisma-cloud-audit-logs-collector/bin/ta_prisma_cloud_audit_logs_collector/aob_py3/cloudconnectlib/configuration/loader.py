...
from ..common.util import load_json_file, is_valid_bool, is_valid_port, is_true
from ..core.exceptions import ConfigException
from ..core.ext import lookup_method
from ..core.models import BasicAuthorization, RequestParams, Processor, Condition, Task, Checkpoint, IterationMode, DictToken
...
class CloudConnectConfigLoader(object):
    ...
    def _get_schema_from_file(schema_file):
        ...
        try:
            ...
            raise ConfigException("Cannot load schema from file {}: {}".format(schema_file, traceback.format_exc()))
...
class CloudConnectConfigLoaderV1(CloudConnectConfigLoader):
    ...
    def _render_from_dict(source, ctx):
        ...
        return dict((k, v.strip() if isinstance(v, six.string_types) else v) for k, v in rendered.items())
    ...
    def _load_proxy(self, candidate, variables):
        ...
        if not is_valid_bool(enabled):
            raise ValueError('Proxy "enabled" expect to be bool type: {}'.format(enabled))
        ...
        if host or port:
            ...
            if not is_valid_port(port):
                raise ValueError('Proxy "port" expect to be in range [1,65535]: %s' % port)

        # proxy type default to 'http'
        proxy_type = proxy.get("type")
        proxy_type = proxy_type.lower() if proxy_type else "http"
        if proxy_type not in _PROXY_TYPES:
            raise ValueError('Proxy "type" expect to be one of [{}]: {}'.format(",".join(_PROXY_TYPES), proxy_type))
        else:
        ...
        if not is_valid_bool(proxy_rdns):
            raise ValueError('Proxy "rdns" expect to be bool type: {}'.format(proxy_rdns))
        else:
    ...
    def _get_log_level(level_name):
        ...
        _logger.warning('The log level "%s" is invalid, set it to default: "%s"', level_name, _DEFAULT_LOG_LEVEL)
    ...
    def _load_authorization(candidate):
        ...
        if auth_type not in _AUTH_TYPES:
            raise ValueError("Auth type expect to be one of [{}]: {}".format(",".join(list(_AUTH_TYPES.keys())), auth_type))
    ...
    def _load_iteration_mode(self, iteration_mode):
        ...
        return IterationMode(iteration_count=iteration_count, conditions=stop_conditions)
    ...
    def load(self, definition, schema_file, context):
        ...
        try:
            ...
            raise ConfigException("Failed to validate interface with schema: {}".format(traceback.format_exc()))

        try:
            global_settings = self._load_global_setting(definition.get("global_settings"), context)
...
def get_loader_by_version(version):
    ...
    raise ConfigException(
        ...
        " versions should match these regex [{}]".format(version, ",".join(_loader_and_schema_by_version))
    )
