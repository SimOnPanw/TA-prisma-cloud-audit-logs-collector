...
def cce_pipeline_plugin(func):
    ...
    if not callable(func):
        logger.debug("Function %s is not callable, don't add it as a pipeline" " function", func.__name__)
    else:
        if func.__name__ in list(_extension_functions.keys()):
            logger.warning("Pipeline function %s already exists, please rename" "it!", func.__name__)
        else:
...
def import_plugin_file(file_name):
    ...
    if file_name.endswith(".py"):
        ...
        logger.warning("Plugin file %s is with unsupported extenstion, the " "supported are py", file_name)
        return

    if module_name in list(sys.modules.keys()):
        logger.warning(
            "Module %s aleady exists and it won't be reload, " "please rename your plugin module if it is required.",
            module_name,
        )
        return
    ...
    try:
        ...
        logger.warning("Failed to load module {}, {}".format(module_name, traceback.format_exc()))
        return
...
def init_pipeline_plugins(plugin_dir):
    ...
    if not op.isdir(plugin_dir):
        logger.warning("%s is not a directory! Pipeline plugin files won't be loaded.", plugin_dir)
        return
