...
class BaseTask(object):
    ...
    def _pre_process(self, context):
        self._execute_handlers(self._skip_pre_conditions, self._pre_process_handler, context, "pre")

    def _post_process(self, context):
        self._execute_handlers(self._skip_post_conditions, self._post_process_handler, context, "post")
...
class CCESplitTask(BaseTask):
    ...
    def configure_split(self, method, source, output, separator=None):
        ...
        self._source = source
        self._process_handler = ProcessHandler(method, arguments, CCESplitTask.OUTPUT_KEY)
...
class CCEHTTPRequestTask(BaseTask):
    ...
    def set_iteration_count(self, count):
        ...
        try:
            ...
            self._max_iteration_count = defaults.max_iteration_count
            logger.warning(
                "Invalid iteration count: %s, using default max iteration count: %s", count, self._max_iteration_count
            )
    ...
    def configure_checkpoint(self, name, content):
        ...
        self._checkpointer = CheckpointManagerAdapter(
            namespaces=name, content=content, meta_config=self._meta_config, task_config=self._task_config
        )
    ...
    def _send_request(client, request):
        try:
            ...
            logger.exception("Error occurred in request url=%s method=%s reason=%s", request.url, request.method, error.reason)
            return None, True
        ...
        if status in defaults.success_statuses:
            if not (response.body or "").strip():
                logger.info(
                    "The response body of request which url=%s and" " method=%s is empty, status=%s.",
                    request.url,
                    request.method,
                    status,
                )
        ...
        error_log = ("The response status=%s for request which url=%s and" " method=%s.") % (
            status,
            request.url,
            request.method,
        )
