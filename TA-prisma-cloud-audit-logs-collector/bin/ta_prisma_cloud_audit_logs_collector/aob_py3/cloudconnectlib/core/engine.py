...
class Job(object):
    ...
    def _get_max_iteration_count(self):
        ...
        return min(default_max_count, mode_max_count) if mode_max_count > 0 else default_max_count
    ...
    def _is_stoppable(self):
        ...
        if self._request_iterated_count >= self._max_iteration_count:
            _logger.info(
                "Job iteration count is %s, current request count is %s," " stop condition satisfied.",
    ...
    def _send_request(self, request):
        ...
        try:
            ...
            _logger.exception(
                "HTTPError reason=%s when sending request to " "url=%s method=%s", error.reason, request.url, request.method
            )
            return None, True
