...
class HTTPResponse(object):
    ...
    def _decode_content(response, content):
        ...
        if charset is None:
            ...
            _logger.info("Unable to find charset in response headers," ' set it to default "%s"', charset)
        ...
        try:
            ...
            _logger.warning("Failure decoding response content with charset=%s," " decode it with utf-8: %s", charset, ex)
...
def get_proxy_info(proxy_config):
    ...
    proxy_info = ProxyInfo(
        proxy_host=url, proxy_port=int(port), proxy_type=ptv, proxy_user=user, proxy_pass=password, proxy_rdns=rdns
    )
    return proxy_info
...
class HttpClient(object):
    ...
    def _send_internal(self, uri, method, headers=None, body=None, proxy_info=None):
        ...
        try:
            return self._connection.request(uri, body=body, method=method, headers=headers)
            ...
            self._connection = self._build_http_connection(proxy_info=proxy_info, disable_ssl_cert_validation=True)
            return self._connection.request(uri, body=body, method=method, headers=headers)
    ...
    def _retry_send_request_if_needed(self, uri, method="GET", headers=None, body=None):
        ...
        for i in range(retries + 1):
            try:
                response, content = self._send_internal(uri=uri, body=body, method=method, headers=headers)
            except Exception as err:
                _logger.exception("Could not send request url=%s method=%s", uri, method)
            ...
            if self._is_need_retry(status, i, retries):
                delay = 2**i
                _logger.warning(
                    "The response status=%s of request which url=%s and" " method=%s. Retry after %s seconds.",
                    status,
                    uri,
                    method,
                    delay,
    ...
    def send(self, request):
        ...
        try:
            ...
            _logger.warning("Failed to encode url=%s: %s", request.url, traceback.format_exc())
            url = request.url
    ...
    def _build_http_connection(
        proxy_info=None, timeout=defaults.timeout, disable_ssl_cert_validation=defaults.disable_ssl_cert_validation
    ):
        return Http(proxy_info=proxy_info, timeout=timeout, disable_ssl_certificate_validation=disable_ssl_cert_validation)
