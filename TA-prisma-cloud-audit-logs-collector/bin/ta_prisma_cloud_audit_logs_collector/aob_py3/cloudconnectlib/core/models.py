...
class _Token(object):
    ...
    def __init__(self, source):
        ...
        self._source = source
        self._value_for = compile_template(source) if isinstance(source, six.string_types) else None
...
class BasicAuthorization(BaseAuth):
    ...
    def __call__(self, headers, context):
        ...
        headers["Authorization"] = (
            "Basic %s" % self.to_native_string(base64.b64encode((username + ":" + password).encode("latin1"))).strip()
        )
...
class RequestParams(object):
    ...
    def render(self, ctx):
        return Request(
            url=self._url.render(ctx), method=self._method, headers=self.normalize_headers(ctx), body=self.body.render(ctx)
        )
...
class Task(_Function):
    ...
    def execute(self, context):
        ...
        _logger.info("Executing task method: [%s], input size: [%s], output: [%s]", self.function, len(args), output)
...
class Condition(_Function):
    ...
    def calculate(self, context):
        ...
        _logger.debug("Calculating condition with method: [%s], input size: [%s]", self.function, len(args))
