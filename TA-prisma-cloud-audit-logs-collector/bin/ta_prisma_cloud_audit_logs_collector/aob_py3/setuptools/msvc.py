...
def _augment_exception(exc, version, arch=""):
    ...
    if "vcvarsall" in message.lower() or "visual c" in message.lower():
        ...
        if version == 9.0:
            ...
            message += (
                ' Get it with "Microsoft C++ Build Tools": ' r"https://visualstudio.microsoft.com" r"/visual-cpp-build-tools/"
            )
...
class SystemInfo:
    ...
    def _use_last_dir_name(path, prefix=""):
        ...
        """
        matching_dirs = (
            dir_name for dir_name in reversed(listdir(path)) if isdir(join(path, dir_name)) and dir_name.startswith(prefix)
        )
