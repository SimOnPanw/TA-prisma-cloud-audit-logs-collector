...
def check_packages(dist, attr, value):
    for pkgname in value:
        if not re.match(r"\w+(\.\w+)*", pkgname):
            distutils.log.warn(
                "WARNING: %r not a valid package name; please use only " ".-separated package names in setup.py", pkgname
            )
...
class Distribution(_Distribution):
    ...
    def _expand_patterns(patterns):
        ...
        """
        return (
            path for pattern in patterns for path in sorted(iglob(pattern)) if not path.endswith("~") and os.path.isfile(path)
        )
