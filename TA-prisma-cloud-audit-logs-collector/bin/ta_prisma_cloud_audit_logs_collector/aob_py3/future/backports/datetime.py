...
def _check_utc_offset(name, offset):
    ...
    if not -timedelta(1) < offset < timedelta(1):
        raise ValueError(
            "%s()=%s, must be must be strictly between" " -timedelta(hours=24) and timedelta(hours=24)" % (name, offset)
        )
...
class timedelta(object):
    ...
    def __add__(self, other):
        if isinstance(other, timedelta):
            ...
            return timedelta(
                self._days + other._days, self._seconds + other._seconds, self._microseconds + other._microseconds
            )
...
class date(object):
    ...
    def __new__(cls, year, month=None, day=None):
        ...
        """
        if isinstance(year, bytes) and len(year) == 4 and 1 <= year[2] <= 12 and month is None:  # Month is sane
            # Pickle support
            self = object.__new__(cls)
...
class datetime(date):
    ...
    def timestamp(self):
        ...
        if self._tzinfo is None:
            return (
                _time.mktime((self.year, self.month, self.day, self.hour, self.minute, self.second, -1, -1, -1))
    ...
    def _cmp(self, other, allow_mixed=False):
        ...
        if base_compare:
            return _cmp(
                (self._year, self._month, self._day, self._hour, self._minute, self._second, self._microsecond),
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, timedelta):
            return NotImplemented
        delta = timedelta(
            self.toordinal(), hours=self._hour, minutes=self._minute, seconds=self._second, microseconds=self._microsecond
        )
        delta += other
        ...
        if 0 < delta.days <= _MAXORDINAL:
            return datetime.combine(
                date.fromordinal(delta.days), time(hour, minute, second, delta.microseconds, tzinfo=self._tzinfo)
            )
        raise OverflowError("result out of range")
