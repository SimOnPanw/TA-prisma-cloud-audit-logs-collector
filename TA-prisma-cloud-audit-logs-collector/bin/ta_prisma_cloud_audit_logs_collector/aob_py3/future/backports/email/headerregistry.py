...
class BaseHeader(str):
    ...
    def fold(self, **_3to2kwargs):
        ...
        header = parser.Header(
            [
                parser.HeaderLabel([parser.ValueTerminal(self.name, "header-name"), parser.ValueTerminal(":", "header-sep")]),
        ...
        if isinstance(value, str):
            ...
            for addr in address_list.addresses:
                groups.append(
                    Group(
                        addr.display_name,
                        [Address(mb.display_name or "", mb.local_part or "", mb.domain or "") for mb in addr.all_mailboxes],
                    )
                )
