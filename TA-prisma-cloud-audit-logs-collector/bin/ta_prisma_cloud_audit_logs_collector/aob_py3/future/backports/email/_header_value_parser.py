...
_non_atom_end_matcher = re.compile(r"[^{}]+".format("".join(ATOM_ENDS).replace("\\", "\\\\").replace("]", "\]"))).match
_non_printable_finder = re.compile(r"[\x00-\x20\x7F]").findall
_non_token_end_matcher = re.compile(r"[^{}]+".format("".join(TOKEN_ENDS).replace("\\", "\\\\").replace("]", "\]"))).match
...
_non_extended_attribute_end_matcher = re.compile(
    r"[^{}]+".format("".join(EXTENDED_ATTRIBUTE_ENDS).replace("\\", "\\\\").replace("]", "\]"))
).match
    ...
    if value and (value[0] == "\\" or value[0] not in PHRASE_ENDS):
        ...
        if obs_local_part.token_type == "invalid-obs-local-part":
            local_part.defects.append(
                errors.InvalidHeaderDefect("local-part is not dot-atom, quoted-string, or obs-local-part")
            )
        else:
...
def parse_mime_version(value):
    ...
    if not digits.isdigit():
        mime_version.defects.append(
            errors.InvalidHeaderDefect("Expected MIME major version number but found {!r}".format(digits))
        )
    ...
    if not digits.isdigit():
        mime_version.defects.append(
            errors.InvalidHeaderDefect("Expected MIME minor version number but found {!r}".format(digits))
        )
        mime_version.append(ValueTerminal(digits, "xtext"))
    else:
    ...
    while value:
        ...
        if value and value[0] != ";":
            ...
            mime_parameters.defects.append(
                errors.InvalidHeaderDefect("parameter with invalid trailing text {!r}".format(token))
            )
        if value:
...
def parse_content_type_header(value):
    ...
    if value[0] != ";":
        ctype.defects.append(
            errors.InvalidHeaderDefect("Only parameters are valid after content type, but " "found {!r}".format(value))
        )
...
def parse_content_disposition_header(value):
    ...
    if value[0] != ";":
        disp_header.defects.append(
            errors.InvalidHeaderDefect("Only parameters are valid after content disposition, but " "found {!r}".format(value))
        )
        _find_mime_parameters(disp_header, value)
        return disp_header
