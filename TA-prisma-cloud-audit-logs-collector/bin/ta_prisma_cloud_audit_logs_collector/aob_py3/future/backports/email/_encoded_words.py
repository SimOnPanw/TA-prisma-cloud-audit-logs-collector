...
# regex based decoder.
_q_byte_subber = functools.partial(re.compile(rb"=([a-fA-F0-9]{2})").sub, lambda m: bytes([int(m.group(1), 16)]))
...
def decode(ew):
    ...
    try:
        ...
        defects.append(
            errors.UndecodableBytesDefect("Encoded word " "contains bytes not decodable using {} charset".format(charset))
        )
        ...
        if charset.lower() != "unknown-8bit":
            defects.append(
                errors.CharsetError("Unknown charset {} " "in encoded word; decoded as unknown bytes".format(charset))
            )
