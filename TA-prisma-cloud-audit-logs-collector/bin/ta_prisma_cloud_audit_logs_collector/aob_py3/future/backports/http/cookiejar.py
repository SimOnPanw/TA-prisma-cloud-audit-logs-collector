...
STRICT_DATE_RE = re.compile(
    r"^[SMTWF][a-z][a-z], (\d\d) ([JFMASOND][a-z][a-z]) " "(\d\d\d\d) (\d\d):(\d\d):(\d\d) GMT$", re.ASCII
)
...
class DefaultCookiePolicy(CookiePolicy):
    ...
    def set_ok_path(self, cookie, request):
        if cookie.path_specified:
            ...
            if (cookie.version > 0 or (cookie.version == 0 and self.strict_ns_set_path)) and not req_path.startswith(
                cookie.path
            ):
