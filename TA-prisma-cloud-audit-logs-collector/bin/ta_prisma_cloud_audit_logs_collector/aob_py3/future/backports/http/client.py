...
class HTTPResponse(io.RawIOBase):
    ...
    def __init__(self, sock, debuglevel=0, strict=_strict_sentinel, method=None, url=None):
        ...
        if strict is not _strict_sentinel:
            warnings.warn(
                "the 'strict' argument isn't supported anymore; " "http.client now always assumes HTTP/1.x compliant servers.",
                DeprecationWarning,
                2,
            )
        self._method = method
    ...
    def begin(self):
        ...
        if status == NO_CONTENT or status == NOT_MODIFIED or 100 <= status < 200 or self._method == "HEAD":  # 1xx codes
            self.length = 0
