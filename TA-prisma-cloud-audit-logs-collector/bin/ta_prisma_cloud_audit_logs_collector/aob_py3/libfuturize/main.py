...
def main(args=None):
    ...
    parser.add_option("-V", "--version", action="store_true", help="Report the version number of futurize")
    parser.add_option("-a", "--all-imports", action="store_true", help="Add all __future__ and future imports to each module")
    parser.add_option(
        "-1",
        "--stage1",
        action="store_true",
    ...
    parser.add_option("-0", "--both-stages", action="store_true", help="Apply both stages 1 and 2")
    parser.add_option(
        "-u",
        "--unicode-literals",
        action="store_true",
    ...
    parser.add_option("-x", "--nofix", action="append", default=[], help="Prevent a fixer from being run.")
    ...
    for fix in options.nofix:
        if ".fix_" in fix:
            ...
            if len(found) > 1:
                ...
                print("Unknown fixer. Use --list-fixes or -l for a list.", file=sys.stderr)
                return 2
    ...
    rt = StdoutRefactoringTool(
        sorted(fixer_names), flags, sorted(explicit), options.nobackups, not options.no_diffs, **extra_kwargs
    )
