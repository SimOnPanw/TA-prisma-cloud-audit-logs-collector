    ...
    def parse(cls, command, argv):
        ...
        for option in cls._options_re.finditer(command_args.group("options")):
            ...
            if name not in command.options:
                raise ValueError("Unrecognized {} command option: {}={}".format(command.name, name, json_encode_string(value)))
...
class ConfigurationSettingsType(type):
    ...
    specification_matrix = {
        ...
        "maxinputs": specification(type=int, constraint=lambda value: 0 <= value <= six.MAXSIZE, supporting_protocols=[2]),
...
class RecordWriter(object):
    ...
    def _record_count(self):
        warnings.warn("_record_count will be deprecated soon. Use pending_record_count instead.", PendingDeprecationWarning)
    ...
    def _total_record_count(self):
        warnings.warn(
            "_total_record_count will be deprecated soon. Use committed_record_count instead.", PendingDeprecationWarning
        )
